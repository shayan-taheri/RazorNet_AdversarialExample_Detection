...
def deepfool(model, x, noise=False, eta=0.01, epochs=3, batch=False, D=0.1,
    ...
    X1 = tf.norm(delta,axis=None,ord=2)
    X2 = tf.norm(x,axis=None,ord=2)

    xadv = tf.stop_gradient(tf.to_double((tf.math.less_equal(tf.div(X1,X2), D))))

    #xadv = tf.stop_gradient(x + delta*(1+eta))
    #if tf.math.less_equal(scr3,D):
